...
(function(global) {
    ...
    // Header -----------------------------------------------
    MeMain["prototype"]["startAfterLoad"] = MeMain_startAfterLoad;
    MeMain["prototype"]["start"] = MeMain_start;
    // private
    MeMain["prototype"]["_touchHandler"] = MeMain__touchHandler;
    MeMain["prototype"]["_drawTouches"] = MeMain__drawTouches;
    MeMain["prototype"]["_drawTouchPoint"] = MeMain__drawTouchPoint;
    MeMain["prototype"]["_devicemotionHandler"] = MeMain__devicemotionHandler;
    MeMain["prototype"]["_resizeCanvasHandler"] = MeMain__resizeCanvasHandler;
    MeMain["prototype"]["_resizeCanvas"] = MeMain__resizeCanvas;
    if (PC_DEBUG) {
        MeMain["prototype"]["_clickHandler"] = MeMain__clickHandler;
        MeMain["prototype"]["_keydownHandler"] = MeMain__keydownHandler;
    }
    ...
    function MeMain__drawTouchPoint(ctx, touch, touchColor) {
        var px = touch.pageX;
        var py = touch.pageY;
    ...
    function MeMain__touchHandler(orgThis, event) {
    ...
    function MeMain__clickHandler(orgThis, event) {
        var touches = [{
            "identifier" : 0,
            "pageX" : 200,
            "pageY" : 220,
    ...
    function MeMain__keydownHandler(orgThis, event) {
    ...
    function MeMain__devicemotionHandler(orgThis, event) {
        ...
        if (this.useShakeOperation && (ACCEL_SQ_THRESHOLD < sq)) {
            this._devicemotionHandler2 = this._devicemotionHandler2
                    || debounce(MeMain__clearTouches, 100);
            this._devicemotionHandler2();
        }
    }

    function MeMain__clearTouches() {
        this._trackingTouches = [];
        this._drawTouches();
    }

    function MeMain_start() {
        var touchElement;
    ...
    function MeMain_startAfterLoad() {
        var self = this;
        if (IN_BROWSER) {
    ...
    // Exports ----------------------------------------------
    if ("process" in global) {
        module["exports"] = MeMain;
    }
    global["MeMain"] = MeMain;
